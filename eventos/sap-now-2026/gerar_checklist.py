...
col_widths = [55, 12, 14, 22, 50]
for i, w in enumerate(col_widths, 1):
    ws.column_dimensions[get_column_letter(i)].width = w

# ── Linha de título ────────────────────────────────────────────────────────────
ws.merge_cells("A1:E1")
ws["A1"] = "CHECKLIST SAP NOW AI TOUR 2026  —  09 e 10/set  |  Transamérica Expo Center"
...
def linhas(ws, row, dados):
    for i, (atv, prazo, status, resp, obs) in enumerate(dados):
        r = row + i
        bg = fill(CINZA_CLARO) if i % 2 == 0 else fill(BRANCO)
        vals = [atv, prazo, status, resp, obs]
...
planejamento = [
    ...
    ("Data final para envio dos conteúdos para a agenda (sessão Klabin)",               "26/jun", "Pendente", "Fran",    "CRÍTICO — prazo fixo SAP"),
    ...
    ("Enviar assinatura do orçamento de anúncios SAP NOW 2026 (P/Hotéis) Eletromidia", "30/jun", "Pendente", "Fran",    ""),
...
cur += 1  # linha vazia entre seções
...
comunicacao = [
    ...
    ("Criação de legendas para posts de divulgação geral",                     "",  "Pendente", "Fran",              "POST 01 Lançamento / POST 02 Estande / POST 03 Amanhã / POST 04 Dia do evento / POST 06 Agradecimento"),
    ...
    ("Enviar e-mail de alinhamento aos envolvidos do evento com as ações",     "",  "Pendente", "Fran",              ""),
...
pre_evento = [
    ("Treinamento do time (kick off interno)",                            "Set/1",  "Pendente", "Fran",      "Após aprovação final do planejamento — abordagem, dor, registro, o que NÃO falar"),
    ("Ativar anúncios DOOH + Google/LinkedIn",                            "07/set", "Pendente", "Fran",      "Iniciar 1 dia antes do evento"),
    ("Credenciamento para entrega do estande",                            "04/set", "Pendente", "Fran",      ""),
...
orcamento = [
    ...
    ("Brindes — Bloco notas cubo BL31 (250)","A definir",         8767.50, "", "", "R$ 35,07/un — 95x95mm, sik screen, azul"),
...
ws2[f"A{total_row}"] = "TOTAL PARCIAL (itens com valor preenchido)"
ws2[f"A{total_row}"].font = Font(bold=True, color=BRANCO, size=10)
ws2[f"A{total_row}"].fill = fill(AZUL_ESCURO)
ws2[f"A{total_row}"].alignment = centro()
ws2[f"A{total_row}"].border = borda()
ws2[f"C{total_row}"] = f"=SUM(C3:C{total_row-1})"
ws2[f"C{total_row}"].font = Font(bold=True, color=BRANCO, size=10)
...
ws2[f"C{total_row}"].border = borda()
...
col_widths3 = [38, 20, 8, 6, 14, 8, 16, 16, 45]
