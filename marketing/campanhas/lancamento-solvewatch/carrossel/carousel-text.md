# Carrossel — Lançamento Solvewatch (sustentação)

**Versão Canva (formato quadrado 1080x1080):** editar em https://www.canva.com/d/H9SKxFqRk2xFGBA | ver em https://www.canva.com/d/L7qFIjNfMLJ41MT
**Versão HTML/PNG local:** `marketing/campanhas/lancamento-solvewatch/carrossel/slides/` (gerada via `gerar.js`, mesmo texto)

**Ângulo:** as 4 dores trabalhadas na fase de sustentação do lançamento (confiabilidade, custos, governança, IA)
**Formato:** Instagram/LinkedIn, 1080x1080
**Persona:** CIO, Head de Dados/BI, COO

---

## Slide 1 — Capa
Seu Datasphere está em produção.
Quem está cuidando dele?

## Slide 2 — Contexto
Depois do go-live, o desafio muda de figura.
Não é mais sobre implementar. É sobre manter o ambiente saudável, previsível e pronto pra evoluir — todos os dias, sem parar.

## Slide 3 — Confiabilidade
A carga falha de madrugada.
O negócio descobre primeiro — antes de qualquer alerta chegar pra quem cuida do ambiente.

## Slide 4 — Custos
O consumo de Capacity Units cresce em silêncio.
A fatura, não. E quando alguém percebe, já é tarde pra agir com previsibilidade.

## Slide 5 — Governança
Várias mãos construíram o ambiente.
Times diferentes, fornecedores diferentes — e ninguém enxerga o todo.

## Slide 6 — Prontidão para IA
A IA não gera valor sozinha.
Sem contexto e sem governança nos dados, todo investimento em IA fica raso.

## Slide 7 — CTA
Solve Watch
Gestão inteligente e contínua do SAP Datasphere.

Seu Datasphere sob gestão. Seus dados prontos para IA.

[logo Solveplan]
Solicitar avaliação do ambiente →

---

## Legenda sugerida (LinkedIn/Instagram)

Depois do go-live do SAP Datasphere, o desafio muda de figura.

Falha que ninguém vê a tempo. Consumo que cresce sem explicação. Ambiente construído por várias mãos, sem dono. E uma IA que promete muito e entrega pouco sem dados governados.

Foi pra isso que criamos o Solve Watch: gestão inteligente e contínua do SAP Datasphere.

Seu Datasphere sob gestão. Seus dados prontos para IA.

👉 [link da landing page]

#SAPDatasphere #SAPBDC #GovernançaDeDados #DataOps
