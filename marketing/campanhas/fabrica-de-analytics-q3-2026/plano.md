# Plano de Campanha — Fábrica de Analytics Solveplan

**Tipo:** Produto/solução
**Período:** Q3 2026 (julho–setembro 2026)
**Responsável:** Francielle Beline
**Data de criação:** 18/06/2026

---

## 1. Objetivo

**Objetivo principal:** Geração de leads qualificados para o serviço de Fábrica de Analytics (suporte técnico e melhorias contínuas no ambiente SAC/Datasphere — banco de horas por demanda, sem projeto)

**Produto / tema central:** Fábrica de Analytics — suporte contínuo e melhorias no ambiente SAC/Datasphere já implantado: corrigir o que travou, evoluir o que existe, sustentar o ambiente em operação. Por demanda, sem aumentar equipe interna.

---

## 2. Público-alvo

**Cargos:** Gerente/Coordenador de TI, Head de Dados/BI, Gestor de FP&A, Controller, CFO
**Segmento:** Indústria, varejo, agronegócio, financeiro
**Tipo de empresa:** Médio e grande porte, em S4HANA ou RISE, com SAC contratado
**Momento de compra:** SAC parado ou subutilizado / arquitetura RFC em risco / S4 sobrecarregado / sem capacidade técnica interna para evoluir o ambiente

**Dores prioritárias — baseadas em VoC de 18 reuniões reais:**

| # | Dor | Frequência | Intensidade | Conexão com suporte/melhorias |
|---|-----|-----------|-------------|-------------------------------|
| 1 | **Sem parceiro técnico para sustentar e evoluir SAC/Datasphere** | 3+ reuniões | **Muito alta** — fala explícita de "suporte" — dor direta do serviço | Tuberfil: "a gente queria uma empresa para dar suporte nessa passagem de conhecimento" |
| 2 | SAC contratado mas parado — sem suporte para ativar | 4 reuniões | Alta — ferramenta implantada, mas sem ninguém para fazer funcionar | Precisam de suporte para ativação e manutenção contínua |
| 3 | RFC em risco — precisam de suporte para migrar a arquitetura | 3 reuniões | Muito alta — urgência real, impacto operacional | Migração + sustentação da nova arquitetura |
| 4 | S4 sobrecarregado — precisam de suporte para mover relatórios | 3 reuniões | Alta — argumento financeiro e técnico | Ajuste e sustentação da camada analítica |
| 5 | SAC usado só como BI, sem planning — precisam de suporte para evoluir | 3 reuniões | Média — oportunidade de expansão de uso | Melhorias incrementais no ambiente existente |

**Linguagem nativa dos clientes (usar nas peças):**
- "a gente queria uma empresa para dar suporte nessa passagem de conhecimento" ← **fala central do serviço**
- "tentamos usar ele aqui, não foi muito bem aceito"
- "a parte de planning a gente não evoluiu. Temos licenciamento..."
- "nós estamos assombrados com isso" (RFC)
- "sem ele basicamente a fábrica pararia"
- "eu preciso desafogar o nosso S4"

**O que NÃO usar (não ressoa com o cliente, baseado no VoC):**
- IA / análise preditiva — nenhuma menção espontânea nas 18 reuniões
- BDC como posicionamento amplo — clientes não reconhecem como problema
- Joule / Copilot SAP — zero menções

---

## 3. Mensagem central

**Proposta de valor:**
Suporte técnico e melhorias contínuas no seu ambiente SAC/Datasphere — sem projeto, por demanda. Você chama quando precisa: para corrigir o que travou, melhorar o que existe ou avançar para o próximo nível.

**4 ângulos de mensagem (um por dor principal):**

**Ângulo 1 — Sem parceiro técnico (dor central do serviço):**
> "Quem dá suporte no seu ambiente SAC hoje? Muitas empresas no RISE têm o ambiente implantado mas sem ninguém para evoluir, corrigir e sustentar. Um banco de horas com especialistas SAP resolve isso sem contratar."

**Ângulo 2 — SAC parado, sem suporte para ativar:**
> "Sua licença do SAC está sendo usada? Não é falta de ferramenta — é falta de suporte técnico para ativar e manter. A Solveplan entra, coloca no ar e sustenta o ambiente sem projeto de meses."

**Ângulo 3 — RFC urgência:**
> "A SAP vai bloquear suas extrações via RFC. Para muitas empresas, isso significa parar relatórios, DWs e até sistemas operacionais. Existe uma rota de transição com suporte especializado que não exige reescrever tudo do zero."

**Ângulo 4 — S4 sobrecarregado:**
> "Seu S4 não é para relatórios. Cada query analítica que roda no transacional afeta performance e aparece na fatura. Com suporte técnico certo, você move isso para a camada correta sem projeto interno."

**Prova / argumento de autoridade:**
- Melhor Parceiro SAP Business Data Cloud 2026 — América Latina
- 150+ empresas com resultados comprovados, 390+ projetos entregues
- Cases: VALE, Petrobras, BTG Pactual, C&A, Usiminas

**CTA principal:**
"Fale com um especialista sobre o suporte ao seu ambiente SAC/Datasphere — 30 minutos, sem compromisso."

---

## 4. Metas de sucesso

| Indicador | Meta Q3 2026 |
|-----------|-------------|
| Alcance (impressões) | 30.000–50.000 |
| Leads gerados | 8–12 |
| Leads qualificados (MQLs) | 4–7 |
| Reuniões agendadas | 2–4 |
| Oportunidades abertas | 1–2 |
| Pipeline gerado | R$ 678k–1,3M |

**Racional:** 1 deal fechado (ticket médio R$ 678k) paga o investimento da campanha inteira com folga. O objetivo do Q3 é criar o primeiro ciclo digital estruturado, gerar aprendizado e escalar no Q4.

---

## 5. Canais e formatos

| Canal | Formato | Frequência | Prioridade |
|-------|---------|------------|------------|
| Email marketing (base warm) | Sequência 3 emails por ângulo de dor | 1x/mês por ângulo | Alta — custo zero, base qualificada |
| LinkedIn orgânico | Posts por ângulo de dor + evidências do VoC | 2–3x/semana | Alta — ICP ativo no LinkedIn |
| LinkedIn Ads | Lead gen form + sponsored content | Sempre ativa | Alta — segmentação precisa do ICP |
| Google Ads | Keywords RFC + SAC Planning + Datasphere | Sempre ativa | Alta — já performa (CPC R$ 2,48) |
| Blog / artigo | 1 artigo por ângulo de dor (SEO) | 1x/mês | Média — médio prazo |
| WhatsApp / direto | Follow-up dos 95 warm leads do H1 | 1x na semana 1 | Urgente — reativação imediata |

**Nota sobre reativação:** Os 95 hot/warm leads dos eventos SAP BTP Experience e IT Summit Agro (H1 2026) são o ativo mais quente da campanha. Contactar antes de qualquer investimento em mídia paga.

---

## 6. Cronograma

### Pré-campanha — até 04/07/2026

| Atividade | Prazo | Responsável | Status |
|-----------|-------|-------------|--------|
| Definir mensagem central e ângulos | 27/06 | Marketing | |
| Mapear e segmentar base de warm leads (95) | 27/06 | CRM/Marketing | |
| Criar sequência de emails (3 por ângulo) | 04/07 | Marketing | |
| Criar 3 posts LinkedIn por ângulo de dor | 04/07 | Marketing | |
| Configurar campanha LinkedIn Ads | 04/07 | Marketing | |
| Configurar/otimizar Google Ads (palavras RFC + SAC) | 04/07 | Marketing | |
| Criar landing page dedicada para a campanha | 04/07 | Marketing | |
| Configurar UTMs e pipeline no HubSpot | 04/07 | Marketing/CRM | |
| Briefar time comercial sobre os ângulos da campanha | 04/07 | Comercial | |

### Durante a campanha

| Semana | Atividades | Canal |
|--------|-----------|-------|
| Semana 1 (07/07) | Reativar 95 warm leads — foco no ângulo RFC (urgência) | WhatsApp + email |
| Semana 2 (14/07) | Lançar posts LinkedIn ângulo 1 (SAC parado) + ativar LinkedIn Ads | LinkedIn |
| Semana 3 (21/07) | Email sequência ângulo RFC + Google Ads ativo | Email + Search |
| Semana 4 (28/07) | Publicar artigo blog ângulo RFC + posts ângulo 2 | Blog + LinkedIn |
| Semana 5 (04/08) | Email ângulo S4 sobrecarregado + revisar performance ads | Email + Ads |
| Semana 6 (11/08) | Posts LinkedIn ângulo capacidade técnica | LinkedIn |
| Semana 7 (18/08) | Email ângulo SAC como BI + artigo blog ângulo SAC parado | Email + Blog |
| Semana 8 (25/08) | Revisão mid-campaign: otimizar canais que performam, pausar o que não performa | Todos |
| Semana 9 (01/09) | Acelerar canal líder + reativar leads não convertidos | Definir em semana 8 |
| Semana 10–12 (set) | Follow-up, qualificação, reuniões comerciais | Comercial |

### Pós-campanha

| Atividade | Prazo | Responsável |
|-----------|-------|-------------|
| Follow-up de leads (SLA: 24h úteis) | Contínuo | Comercial |
| Reunião de resultado Q3 | 30/09 | Marketing + Comercial |
| Relatório de performance | 07/10 | Marketing |
| Registro de aprendizados para Q4 | 07/10 | Marketing |

---

## 7. Budget

**Total disponível:** A definir

| Item | Estimativa sugerida |
|------|-------------------|
| LinkedIn Ads | R$ 3.000–6.000/mês |
| Google Ads | R$ 1.500–3.000/mês |
| Produção de peças (banners, LP) | R$ 1.500–2.500 (único) |
| **Total estimado Q3** | **R$ 16.000–29.000** |

**Nota:** O Google Ads atual já tem CPC R$ 2,48 e CTR 4,97% — é o canal de menor custo e maior previsibilidade. LinkedIn Ads é mais caro mas permite segmentar por cargo + empresa SAP com precisão cirúrgica.

---

## 8. Peças necessárias

| Peça | Formato | Canal | Prioridade |
|------|---------|-------|-----------|
| Email sequência "sem parceiro técnico" (3 emails) | Texto | Email marketing | Alta — dor central do serviço |
| Email sequência RFC urgência (3 emails) | Texto | Email marketing | Alta |
| Email sequência SAC parado (3 emails) | Texto | Email marketing | Alta |
| 4 posts LinkedIn (um por ângulo de dor) | Copy + visual | LinkedIn orgânico | Alta |
| Banner LinkedIn Ads (3 variações A/B) | Visual + copy | LinkedIn Ads | Alta |
| Landing page: suporte SAC/Datasphere por demanda | HTML/copy | Paid + orgânico | Alta |
| Artigo: "RFC bloqueado — o que muda no seu ambiente SAP" | Blog | SEO + LinkedIn | Média |
| Artigo: "Quem sustenta seu SAC depois da implantação?" | Blog | SEO + LinkedIn | Média |
| Copy Google Ads (suporte SAC + RFC + Datasphere) | Texto | Google Ads | Alta |

*Use `/material-campanha` para briefing e copy de cada peça.*
*Use `/artigo-blog` para os dois artigos.*
*Use `/post-social` para os posts LinkedIn.*
*Use `/anuncio` para os banners e copy de ads.*

---

## 9. Integração comercial

**Pipeline no CRM:** HubSpot — criar deal na etapa "MQL" ao qualificar lead
**SLA de contato pós-lead:** 24 horas úteis
**Critério de qualificação (MQL):**
- Empresa em S4HANA ou RISE com SAC contratado **E**
- Cargo decisor (TI, FP&A, Controladoria, CFO) **E**
- Uma das dores confirmadas: SAC parado / RFC em risco / S4 sobrecarregado

**Script de qualificação para o comercial:**
1. "Vocês têm SAC contratado? Está em uso para planning ou só para dashboards?"
2. "Sua arquitetura de dados usa RFC para alimentar DW ou Power BI?"
3. "Quem faz o desenvolvimento e evolução do SAC internamente hoje?"

---

## 10. KPIs de acompanhamento

| KPI | Jul | Ago | Set | Q3 Total |
|-----|-----|-----|-----|----------|
| Leads gerados | 3–4 | 3–4 | 2–4 | 8–12 |
| MQLs | 1–2 | 2–3 | 1–2 | 4–7 |
| Reuniões | 0–1 | 1–2 | 1–2 | 2–4 |
| CPL (custo por lead) | — | — | — | < R$ 2.500 |
| Taxa MQL/Lead | — | — | — | > 50% |
| Pipeline | — | — | — | R$ 678k–1,3M |

---

## 11. Aprendizados pós-campanha

*(Preencher ao final do Q3)*

**O que funcionou:**

**O que não funcionou:**

**O que fazer diferente no Q4:**
