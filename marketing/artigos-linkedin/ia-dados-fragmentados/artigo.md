...
**O problema não é o modelo de IA**

A tentação é sempre resolver pela ponta mais visível: qual modelo usar, qual ferramenta comprar, qual agente configurar. Mas o modelo de IA não cria contexto de negócio que não existe na base de dados.

Quando os dados estão espalhados entre SAP e sistemas de terceiros, cada um com uma definição diferente pra "cliente" ou "margem líquida", a IA generativa não resolve essa divergência — ela amplifica. Alguns especialistas já chamam isso de GIAGO: garbage in, amplified garbage out. Viés e inconsistência não ficam contidos no modelo — saem maiores do que entraram.

A McKinsey (State of AI, 2025) confirma esse padrão: 78% das empresas já usam IA em pelo menos uma função de negócio, mas cerca de dois terços continuam travadas em piloto. Dados fragmentados e tecnologia legada aparecem como um dos principais bloqueadores de escala — ao lado de processos não redesenhados e falta de priorização clara.

**A confiança nos dados está caindo, não subindo**

Um dado que me chamou atenção recentemente: segundo pesquisa da Salesforce com líderes de negócio, a confiança na confiabilidade dos próprios dados caiu de 54% em 2023 para 40% em 2025. A percepção de precisão caiu de 49% para 36% no mesmo período.

Isso é contraintuitivo — a expectativa era que mais tecnologia gerasse mais confiança. O que está acontecendo é o oposto: quanto mais IA é usada sobre dados fragmentados, mais visível fica a fragilidade da base.
...
Se esse é um problema que você reconhece de dentro, a Solveplan avalia a fundação de dados da sua empresa antes do próximo projeto de IA — com diagnóstico e plano de ação claros.
