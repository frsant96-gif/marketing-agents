...
def faq_id():
    return "faq-question-" + uuid.uuid4().hex[:8]
...
# Build Yoast FAQ block
questions_json = json.dumps([{"id": f["id"], "jsonQuestion": f["question"], "jsonAnswer": f["answer"]} for f in faqs])

faq_html_items = ""
for f in faqs:
    faq_html_items += f"""<div class="schema-faq-section" id="{f['id']}"><strong class="schema-faq-question">{f['question']}</strong><p class="schema-faq-answer">{f['answer']}</p></div>"""

yoast_faq_block = f"""<!-- wp:yoast/faq-block {{"questions":{questions_json}}} -->
<div class="schema-faq wp-block-yoast-faq-block">{faq_html_items}</div>
<!-- /wp:yoast/faq-block -->"""

# Full post content replacing the H3-based FAQ with Yoast block
