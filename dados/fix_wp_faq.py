...
questions_json = json.dumps([
    {"id": f["id"], "jsonQuestion": f["question"], "jsonAnswer": f["answer"]}
    for f in faqs
], ensure_ascii=False)
...
for f in faqs:
    html_items += (
        f'<div class="schema-faq-section" id="{f["id"]}">'
        f'<strong class="schema-faq-question">{f["question"]}</strong>'
        f'<p class="schema-faq-answer">{f["answer"]}</p>'
        f'</div>'
    )

faq_block = (
    f'<!-- wp:yoast/faq-block {{"questions":{questions_json}}} -->\n'
    f'<div class="schema-faq wp-block-yoast-faq-block">{html_items}</div>\n'
    f'<!-- /wp:yoast/faq-block -->'
)

# Fetch current content and replace only the FAQ block
resp = requests.get(
...
# Replace the existing yoast/faq-block section
import re
new_content = re.sub(
...
faq_start = saved.find("schema-faq-section")
snippet = saved[faq_start:faq_start+300] if faq_start > 0 else "nao encontrado"
